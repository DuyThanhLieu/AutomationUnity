 ...
 *
 * CHẠY:
 *   npx playwright test tests/standard/2026-Q3-task-3462/afc-champions-league/07-match-events.spec.ts --project=chrome
 */
...
test.describe(`[${SEASON_DIR}] [Chuẩn #16] Sự kiện trận đấu — ${NAME}`, () => {
  ...
  test('Bàn thắng trong opta_match_event khớp với tỷ số thật trong sport_event_status', async () => {
    // AFC Champions League là cúp knockout — nhiều trận vòng loại trực tiếp
    // có thể vào hiệp phụ/luân lưu (đã verify qua dữ liệu thật: 1 trận có
    // regular_score=3-3 nhưng tổng event goal=15 do gồm cả period_id=5
    // (luân lưu, 9 quả) + period 1/2 (hiệp chính)). Cùng nguyên nhân đã xác
    // nhận ở Eredivisie: so TOÀN BỘ event goal với regular_score (chỉ tính
    // hiệp chính) là sai bản chất. Sửa: chỉ đếm event ở period_id IN (1,2).
    ...
    expect.soft(matchPct, `Chỉ ${matchPct.toFixed(1)}% trận (full coverage) khớp số bàn thắng — sau khi đã loại trừ hiệp phụ/luân lưu, tỷ lệ này phải rất cao`).toBeGreaterThanOrEqual(80);
  });

  test('VAR — quyết định trọng tài (decision/outcome) thuộc tập giá trị hợp lệ, phát hiện trùng lặp event', async () => {
    ...
    // xuất hiện >1 lần — dấu hiệu lỗi insert trùng (đã xác nhận là bug thật
    // ở tầng DB cho Eredivisie, nhưng KHÔNG lộ ra UI — chỉ log cảnh báo).
    ...
    if (duplicateKeys.length > 0) {
      console.warn(`⚠ Nghi vấn: ${duplicateKeys.length} nhóm sự kiện VAR bị trùng lặp hoàn toàn (cùng match/player/time/type) — cần kiểm tra thêm xem có lộ ra UI không (chỉ log, chưa assert cứng).`);
    }
  });
  ...
  test('Thẻ vàng/đỏ trong sport_event_status đối chiếu chéo với opta_match_card', async () => {
    ...
    // yellow_card/red_card) và Opta (opta_match_card — event chi tiết từng
    // thẻ). Công thức: yellow_card = count(YC) - count(Y2C); red_card =
    // count(RC) + count(Y2C). Assert soft do biến động tự nhiên (VAR hủy
    // thẻ/re-sync) — đã xác nhận qua Eredivisie dao động 65-96%.
  ...
  test('Frontend API thật (incidents) — sự kiện trận đấu hiển thị đúng cấu trúc dữ liệu', async ({ page, request }) => {
    ...
    // Chỉ verify được khi frontend đang index ít nhất 1 trận thật của giải.
