 ...
 *
 * CHẠY:
 *   npx playwright test tests/standard/2026-Q3-task-3462/afc-champions-league/10-multilanguage.spec.ts --project=chrome
 */
...
test.describe(`[${SEASON_DIR}] [Chuẩn #11/#20] Đa ngôn ngữ — ${NAME}`, () => {
  ...
  test('Tên đội có bản dịch đầy đủ 6 ngôn ngữ khác (ja, ko, zht, de, fr, es)', async () => {
    ...
    // thống hỗ trợ. Cột tiếng Trung ĐÚNG là "name_zht" (phồn thể) — đã xác
    // nhận cho Eredivisie, giữ nguyên logic cho AFC CL.
    const rows = await withClient(async (client) => {
      const res = await client.query(
  ...
  test('team_locale.name_en đối chiếu chéo với ts_teams.name — cùng 1 đội, không lệch sang đội khác', async () => {
    ...
    // cùng team id) — dùng fuzzy match dạng substring (sau khi chuẩn hoá bỏ
    // khoảng trắng/ký tự đặc biệt), giống Eredivisie, vì name_en hợp lệ có
    // thể là tên rút gọn. Phát hiện lỗi nghiêm trọng hơn: name_en trỏ NHẦM
    // SANG TÊN ĐỘI KHÁC HẲN (do lệch ID khi đồng bộ locale).
    // AFC CL (giải quốc tế, nhiều đội tên dài) lộ thêm 1 lớp viết tắt hợp lệ
    // Eredivisie không có: "United"→"Utd", "Football Club"→"FC" — đã verify
    // qua 3 case biên (Buriram United/Utd, Adelaide United/Utd, Gwangju
    // Football Club/FC) là biến thể tên hợp lệ, không phải lỗi lệch ID. Fold
    // các viết tắt phổ biến này trước khi so substring.
