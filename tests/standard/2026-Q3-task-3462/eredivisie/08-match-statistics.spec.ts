 ...
 *
 * CHẠY:
 *   npx playwright test tests/standard/2026-Q3-task-3462/eredivisie/08-match-statistics.spec.ts --project=chrome
 */
...
test.describe(`[${SEASON_DIR}] [Chuẩn #19] Thống kê — ${NAME}`, () => {
  test('Possession, Passes accuracy, Shots on goal hợp lệ', async () => {
    // SỬA LỖI (đã tự điều tra và xác nhận false positive trước đó): cột
    // "passes_accuracy" KHÔNG lưu tỷ lệ % như tên gợi ý — đã verify qua
    // dữ liệu thật: giá trị dao động 0-803 (kiểu smallint), luôn <= cột
    // "passes" tương ứng. Đây là SỐ LƯỢT CHUYỀN CHÍNH XÁC (count), giống
    // pattern "crosses_accuracy"/"long_balls_accuracy" trong cùng bảng
    // (cũng đã verify là count, không phải %). Muốn ra đúng % phải tính
    // passes_accuracy/passes — verify cách này cho tỷ lệ 64-91%, hợp lý
    // thực tế bóng đá. Case cũ so trực tiếp giá trị với khoảng [0,100] là
    // SAI BẢN CHẤT cột dữ liệu, không phải bug data thật.
    ...
    const results = rows.map((r) => {
      ...
      // accValid: passes_accuracy (count) phải <= passes (tổng số đường
      // chuyền) — không thể có nhiều lượt chuyền chính xác hơn tổng lượt
      // chuyền thực hiện.
  ...
  test('opta_match_stat — mỗi field cộng dồn hiệp 1 (fh) + hiệp 2 (sh) phải bằng "value"', async () => {
    ...
    // fh+sh=value — vì % của hiệp 1 + % của hiệp 2 không có ý nghĩa cộng dồn
    // (vd possession 70% hiệp 1 + 75% hiệp 2 ≠ 145%). Đã phát hiện qua chạy
    // thử: 100% mismatch ban đầu (30/30) đều rơi vào possessionPercentage —
    // xác nhận đây là field duy nhất kiểu % trong bộ stat, không phải bug.
    ...
    for (const row of statRows) {
      for (const [fieldName, fieldValue] of Object.entries(row.stat as Record<string, any>)) {
        ...
        if (isNaN(fh) || isNaN(sh) || isNaN(value)) continue; // field không phải dạng số (vd string stat) — bỏ qua

        totalFieldsChecked++;
        if (Math.abs(fh + sh - value) > 0.01) {
          mismatchCount++;
    ...
    console.log(`\n📊 opta_match_stat fh+sh=value ${NAME}: ${totalFieldsChecked} field kiểm tra trên ${statRows.length} team-match, lệch ${mismatchCount} (${(100 - validPct).toFixed(2)}%)`);
    mismatchSamples.forEach((m) => console.log(`  ✗ ${m.matchId}/${m.teamId} [${m.field}]: fh=${m.fh} + sh=${m.sh} = ${m.fh + m.sh}, nhưng value=${m.value}`));

    saveJsonForSeason(SEASON_DIR, SLUG, `match-statistics-fh-sh-consistency.json`, { totalFieldsChecked, mismatchCount, validPct, mismatchSamples });

    expect(totalFieldsChecked, 'Không có field số nào để kiểm tra trong opta_match_stat').toBeGreaterThan(0);
    expect(mismatchCount, `${mismatchCount}/${totalFieldsChecked} field có fh+sh KHÔNG khớp value — lỗi tính toán trong opta_match_stat`).toBe(0);
  });

  test('Frontend API thật (statistics) — thống kê trận đấu hiển thị đúng cấu trúc dữ liệu', async ({ page, request }) => {
    // Cùng giới hạn tầng ID đã ghi nhận ở #16/#18: endpoint /statistics đòi
    // hỏi thêm home/away team ID (tầng 3, khác team_id trong DB) trong path
    // — vd /event/{eventId}/home/{homeId}/away/{awayId}/statistics — không
    // suy ra được chỉ từ eventId nên phải bắt trực tiếp URL request thật khi
    // trang trận mở ra (discoverFrontendStatsUrl network-capture). SKIP nếu
    // giải hiện không có trận nào được frontend index (đã xác nhận qua thử
    // nghiệm chéo với giải khác đang active — không riêng do off-season).
    const statsUrl = await discoverFrontendStatsUrl(page, SLUG);

    if (!statsUrl) {
      console.warn(`⚠ SKIP: "${NAME}" hiện không có trận nào được frontend index (khả năng off-season, hoặc trang competition-detail không load lịch trận) — không bắt được URL statistics thật.`);
