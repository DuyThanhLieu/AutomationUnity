/**
 * BỘ KHUNG CHUẨN — Hạng mục #6: Chi tiết trận đấu (Tổng quan, H2H)
 *
 * QUAN TRỌNG: dùng bảng "sport_events" — KHÔNG dùng "am_football_sport_events"
 * (đã xác nhận bảng đó chứa dữ liệu American Football/NCAA, không phải bóng
 * đá — xem ALLH2H.spec.ts để biết chi tiết phát hiện bug này).
 *
 * CHẠY:
 *   TEST_COMPETITION_ID=jednm9whz0ryox8 npx playwright test tests/standard/04-h2h.spec.ts --project=chrome
 */
...
test.describe('[Chuẩn #6] H2H — Lịch sử đối đầu', () => {
  ...
  test('Tính H2H từ các trận đã kết thúc trong DB', async () => {
    ...
    expect(matches.length, `Không có trận đã kết thúc nào cho competitionId "${competitionId}"`).toBeGreaterThan(0);
    ...
    console.log(`\n📊 H2H (competitionId=${competitionId}): ${matches.length} trận, ${h2hRecords.length} cặp đối đầu`);
    ...
    saveJson(`h2h.json`, { totalMatches: matches.length, h2hRecords });
  ...
  test('Thống kê tổng quan: home win rate, tổng bàn thắng, TB bàn/trận', async () => {
    ...
    console.log(`\n📊 Tổng quan: ${totalMatches} trận, home_wins=${stats.home_wins}, draws=${stats.draws}, TB bàn/trận=${avgGoals.toFixed(2)}`);

    saveJson(`h2h-overview.json`, { ...stats, avgGoalsPerMatch: avgGoals.toFixed(2) });

    // TB bàn/trận của bóng đá thường trong khoảng 1-6; ngoài khoảng này báo
    // hiệu lỗi tính toán hoặc query sai bảng (bài học từ bug am_football_*).
